...
export default function LoginPage() {
  ...
  const handleEmailLogin = async (formData: FormData) => {
    ...
    try {
      ...
      // Redirect errors are thrown as NEXT_REDIRECT — they're expected
      setIsLoading(false);
    }
  };
      ...
      <div className="flex-1 flex flex-col lg:flex-row relative z-20 max-w-[1600px] mx-auto w-full min-h-screen">
        ...
        <div className="flex-1 flex flex-col justify-center items-center p-8 lg:p-12 relative min-h-[50vh] lg:min-h-screen">
          ...
          >
            ...
            <h1 className="font-serif text-3xl lg:text-4xl font-semibold text-kavach-dark leading-snug mb-3">
              ...
              <span className="text-kavach-brown">Built for a Safer Tomorrow.</span>
            </h1>
            <p className="text-kavach-dark/60 font-sans text-base max-sm">
              Kavach learns, adapts, and protects so you can sign contracts with absolute confidence.
            </p>
          </motion.div>
        </div>
        ...
        <div className="flex-1 flex flex-col justify-center items-center p-8 lg:p-12 min-h-[60vh] lg:min-h-screen">
          ...
          <div className="absolute top-8 right-8 flex items-center gap-4 text-sm font-medium z-30">
            <span className="text-kavach-dark/60 hidden sm:block">New to Kavach?</span>
            <Link href="/register" className="text-kavach-brown hover:text-kavach-gold transition-colors text-base font-semibold">
              Create Account
            </Link>
          </div>
          ...
          >
            ...
            <div className="relative z-10 text-center mb-8">
              <h2 className="font-serif text-4xl font-semibold text-kavach-dark mb-2">Welcome Back</h2>
              <p className="text-kavach-dark/60 text-sm">Securely access your Kavach Shield</p>
            </div>
            ...
            <form className="relative z-10 space-y-5" action={handleEmailLogin}>
              ...
              <div className="flex items-center justify-between text-sm mt-5 mb-6">
                <label className="flex items-center gap-2 cursor-pointer group">
                  <div 
                    className={`w-5 h-5 rounded flex items-center justify-center transition-colors border ${
                      rememberMe ? 'bg-kavach-brown border-kavach-brown' : 'bg-white border-kavach-gold/30 group-hover:border-kavach-gold'
                    }`}
                    onClick={() => setRememberMe(!rememberMe)}
                  >
                    {rememberMe && <CheckCircle className="w-3.5 h-3.5 text-white" strokeWidth={3} />}
                  </div>
                  <span className="text-kavach-dark/70 font-medium select-none">Remember me</span>
                </label>
                <Link href="/forgot" className="text-kavach-brown hover:text-kavach-gold font-medium transition-colors">
                  Forgot Password?
                </Link>
              </div>
              ...
              >
                ...
                <div className="flex items-center gap-3 relative z-10">
                  ...
                  <span className="text-base tracking-wide">
                    {isLoading ? "Signing in..." : "Access Kavach"}
                  </span>
                </div>
              ...
              <div className="mt-8 relative">
                ...
                <div className="relative flex justify-center text-[11px] font-bold tracking-widest text-kavach-dark/40 uppercase">
                  <span className="bg-[#fcfaf7] px-3 rounded-full">Or continue with</span>
                </div>
              </div>
                ...
                >
                  {isGoogleLoading ? (
                    ...
                    <>
                      ...
                      <span className="text-kavach-dark/70 font-medium text-sm">Sign in with Google</span>
                    </>
                  )}
                </button>
              </div>
