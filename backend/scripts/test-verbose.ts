...
async function runVerboseTest() {
  ...
  try {
    ...
    const { data: analysis } = await supabaseAdmin
      .from('analyses')
      .select('overall_risk_score, overall_risk_level, summary, enkrypt_hallucination_score, enkrypt_explanation')
      .eq('id', sessionId)
      .single();
    ...
    console.log(`ENKRYPT HALLUCINATION SCORE: \x1b[1m${analysis?.enkrypt_hallucination_score} / 100\x1b[0m`);
    console.log(`ENKRYPT EXPLANATION: \x1b[33m${analysis?.enkrypt_explanation}\x1b[0m`);
    console.log(`\nSUMMARY:\n${analysis?.summary}`);
    console.log(`======================================================\n`);
